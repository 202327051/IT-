...
def register():
    data = request.json
    username, password = data.get('username'), generate_password_hash(data.get('password'))
    try:
        conn = sqlite3.connect(DB_PATH)
...
# セッション開始（古い今回の回答データを区別するため）
@app.route('/start_session', methods=['POST'])
@login_required
def start_session():
    session['current_session_id'] = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
...
def get_question():
    ...
    if mode == "1":
        res["choices"] = [c.strip() for c in str(q["選択肢"]).split('\n') if c.strip()]
    return jsonify(res)
...
def check_answer():
    ...
    df = pd.read_csv('ITパスポート.csv' if mode == '1' else '用語説明.csv', encoding="utf-8")
    ...
    if mode == "1":
        ...
        score, max_score = len(hit), int(q["満点"])
        res.update({"score": score, "max": max_score, "correct": str(q["模範解答"]), "keywords": keywords, "miss": [k for k in keywords if normalize_text(k) not in user_norm]})
    
    conn = sqlite3.connect(DB_PATH)
    conn.execute("INSERT INTO history (user_id, 問題ID, ジャンル, 回答, 得点, 満点, mode, session_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                 (current_user.id, int(q["id"]), q["ジャンル"], str(user_ans), score, res["max"], mode, session.get('current_session_id')))
    conn.commit()
    return jsonify(res)
...
def get_final_stats():
    ...
    conn = sqlite3.connect(DB_PATH)
    row = conn.execute("SELECT SUM(得点), SUM(満点) FROM history WHERE user_id = ? AND session_id = ?", (current_user.id, sid)).fetchone()
    ...
    conn.commit()
    df_genre = pd.read_sql_query("SELECT ジャンル, SUM(得点) AS s, SUM(満点) AS m, ROUND(SUM(得点)*100.0/SUM(満点), 1) AS rate FROM history WHERE user_id=? AND session_id=? GROUP BY ジャンル ORDER BY rate ASC", conn, params=(current_user.id, sid))
