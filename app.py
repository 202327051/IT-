...
app.config['SECRET_KEY'] = 'it-pass-secret-2026' # セッション管理に必要
CORS(app)
...
def get_final_stats():
    ...
    conn.commit()
    df_genre = pd.read_sql_query("SELECT ジャンル, SUM(得点) AS s, SUM(満点) AS m, ROUND(SUM(得点)*100.0/SUM(満点), 1) AS rate FROM history WHERE user_id=? AND mode='1' GROUP BY ジャンル ORDER BY rate ASC", conn, params=(current_user.id,))
    conn.close()
